...
| `navigation`              | All                 | Opens a workspace or agent on the selected host.                                                                |
...
`navigation.openWorkspace({ workspaceId })` focuses a workspace. `navigation.openAgent({ agentId, workspaceId? })` focuses an agent; pass `workspaceId` when the agent has not reached the client cache yet. `navigation.openNewWorkspace({ serverId?, sourceDirectory?, projectId?, displayName?, provider?, model? })` opens the New workspace screen with optional project and agent defaults. Current hosts also expose `navigation.openWorkspaceAgentCreation(...)` for a combined workspace-plus-agent request: it opens a draft tab immediately, then retargets that tab when `agentCreation.result` resolves. An agent callback may open either an agent panel or a workspace panel. A workspace callback may open only a workspace panel. Unknown surface and panel IDs fail visibly. Use `paseo` for normal workspace, agent, provider, and daemon-config operations. Use `rpc` for plugin-specific filesystem, credential, vendor, or daemon-local work.
